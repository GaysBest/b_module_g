...
var dmip = "46.174.52.3:27269/b_160_400_0_FFFFFF_FFFFFF_FFFFFF_0D0E12_0_1_0.png";
    if(server === "jail") {
        message.channel.send(`http://cache.gametracker.com/server_info/${jailip}.Date.now()`);
    }
    if(server === "dr") {
        message.channel.send(`http://cache.gametracker.com/server_info/${drip}.Date.now()`);
    }
    if(server === "ff2") {
        message.channel.send(`http://cache.gametracker.com/server_info/${ff2ip}.Date.now()`);
    }
    if(server === "dm") {
        message.channel.send(`http://cache.gametracker.com/server_info/${dmip}.Date.now()`);
    }
}
